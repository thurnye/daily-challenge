...
const computeRemainder = (x,y) =>  x % y ;
computeRemainder(10,2) ;//=> 0
...
const reverseUpcaseString = (x) =>  x.split('').reverse().join('').toUpperCase();
...
//took a peep at the solutions to understand :)
...
const formatWithPadding = (num1, str, num2) => {
  ...
  for(let i = numStr.length; i < num2; i++){
    numStr = str + numStr
  }
 return numStr
}
...
const  mumble = (str) => {
  let letters = '';
  for(let i = 0; i < str.length; i++){
letters += ((i || '') && '-') + str.charAt(i).repeat(i + 1);  }
return letters
}

console.log(mumble('abcdef')) //=> 'a-bb-ccc'
